...
def create_power_consumption_boxplot(data_dirs, output_path, custom_labels=None, trim_seconds=0):
    ...
    for dir_path in data_dirs:
        ...
        # The repetition/session directory is the parent of the scenario
        # directory (e.g. Container/0_run2/20250923_..._docker_tools ->
        # run_label "0_run2"), tagged so per-run means can be computed
        # before any cross-repetition averaging.
        run_label = dir_path.parent.name
        ...
        for file_path in power_files:
            ...
            try:
                ...
                if 'rittal' in file_path.name.lower():
                    scenario_rittal[scenario_key].append((run_label, power_values))
                else:
                    scenario_powercap[scenario_key].append((run_label, power_values))
    ...
    # Boxplots show the full pooled raw-sample distribution (unaffected by
    # the per-run-mean treatment used for the printed statistics below).
    rittal_data = []
    rittal_labels = []
    for scenario_key, runs in ordered_rittal:
        if runs:
            combined = pd.concat([series for _run_label, series in runs], ignore_index=True)
    ...
    # Collapse each scenario's (run_label, series) tuples to one scalar per
    ...
    def _collapse_per_run(runs):
        ...
        return {rl: float(pd.concat(s_list, ignore_index=True).mean()) for rl, s_list in per_run.items()}

    rittal_per_run = {k: _collapse_per_run(v) for k, v in scenario_rittal.items()}
    powercap_per_run = {k: _collapse_per_run(v) for k, v in scenario_powercap.items()}

    # Print statistics
    print("\nStatistics (mean +/- std across repetitions; median of per-run means):")
    print("-" * 50)

    if rittal_per_run:
        print("\nEM (Rittal) Measurements:")
        print("-" * 20)
        for scenario_key, _ in ordered_rittal:
            vals = list(rittal_per_run.get(scenario_key, {}).values())
            if not vals:
                continue
            ...
            print(f"\n{label}:")
            print(f"Mean: {fmt_mean_std(stats['mean'], stats['std'])} (n={stats['n']})")
            print(f"Median (of per-run means): {median_value:.2f} Watt")

    if powercap_per_run:
        print("\nRAPL Measurements:")
        print("-" * 20)
        for scenario_key, _ in ordered_powercap:
            vals = list(powercap_per_run.get(scenario_key, {}).values())
            if not vals:
                continue
            stats = summarize_repetitions(vals)
            median_value = float(np.median(vals))
    ...
    # (a) Same-execution paired comparison: P_EM vs P_S within each scenario
    # (both recorded during the same run, so pairing by run label is valid
    # regardless of when different scenarios were collected relative to
    # each other). Individual per-scenario tests are stuck at n=3 (Wilcoxon
    # p-floor=0.25), so also pool the paired differences across all
    # scenarios that have both EM and RAPL data -- scenario is the pooling
    ...
    for scenario_key in scenario_order:
        ...
        if not em or not s:
            continue
        common_runs = sorted(set(em) & set(s))
        ...
        label = scenario_labels.get(scenario_key, scenario_key)
        print(f"% Statistical note ({label}): P_EM vs P_S, same-execution paired (n={wilcoxon['n_nonzero']}):")
