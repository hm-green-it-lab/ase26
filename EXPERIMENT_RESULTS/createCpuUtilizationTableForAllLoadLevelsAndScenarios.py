...
def generate_table_for_env(exp_results, env_name):
    ...
    # Per-run deviation from CPU_none's own mean *at that load level*,
    # pooled across all load levels, per scenario suffix -- used below for
    # the tool-vs-CPU_none statistical notes. CPU utilization scales
    # strongly with load (0% up to ~90%+), which dwarfs the actual
    # per-tool overhead (a few percentage points) if raw values were
    # pooled across load levels as independent samples -- exactly the
    ...
    for load_level in numeric_loads:
        ...
        for suffix, _ in scenario_columns:
            ...
            if is_vm_env:
                # Each entry in host_cpu_utils/vm_cpu_utils is already one
                # repetition's per-run mean CPU utilization, so this list is
                # exactly the per-run granularity summarize_repetitions expects.
                host_stats = summarize_repetitions(host_cpu_utils)
                vm_stats = summarize_repetitions(vm_cpu_utils)
                host_row.append(fmt_mean_std(host_stats["mean"], host_stats["std"], unit="\\%"))
                vm_row.append(fmt_mean_std(vm_stats["mean"], vm_stats["std"], unit="\\%"))

            else:
                cpu_stats = summarize_repetitions(cpu_utils)
                row.append(fmt_mean_std(cpu_stats["mean"], cpu_stats["std"], unit="\\%"))
                if suffix == "docker_tools":
                    baseline_mean_this_level = cpu_stats["mean"]
                elif baseline_mean_this_level is not None:
                    pooled_deviation_by_suffix[suffix].extend(v - baseline_mean_this_level for v in cpu_utils)
    ...
    if not is_vm_env:
        ...
        # --- Statistical notes: each tool's CPU utilization overhead vs ---
        # --- CPU_none, pooled across all load levels as the deviation ---
        # --- from that load level's own CPU_none mean (see comment above ---
        # --- pooled_deviation_by_suffix for why raw pooling is unsuitable). ---
