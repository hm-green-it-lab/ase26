...
def run_local_rittal_reader(
    ...
    interval: str = "*/1 * * * * ?",
    rittal_snmp_address: str = "",
    rittal_snmp_community: str = "",
    rittal_snmp_oids: str = "",
) -> None:
    """
    ...
    if not rittal_snmp_address:
        print("[!] rittal_snmp_address is not configured – Rittal reader will fail to connect.")
    if duration <= 0:
    ...
    if rittal_snmp_address:
        java_cmd.append(f"-Drittal.snmp.address={rittal_snmp_address}")
    if rittal_snmp_community:
        java_cmd.append(f"-Drittal.snmp.community={rittal_snmp_community}")
    if rittal_snmp_oids:
        java_cmd.append(f"-Drittal.snmp.oids={rittal_snmp_oids}")
